...
<script lang="ts">
  ...
  const r1MenuItems: MenuItem[] = [
    ...
    { icon: '🎯', label: 'Grenser og kontinuitet', href: '/r1#grenser-og-kontinuitet' },
    ...
    { icon: '➗', label: 'Differensiallikninger', href: '/r1#differensiallikninger' },
    { icon: '📐', label: 'Vektorer', href: '/r1#vektorer-geometri' },
  ];
  
  $: menuItems = $page.url.pathname.startsWith('/r1') ? r1MenuItems : defaultMenuItems;
...
>
  ...
  {#if !collapsed && currentSection}
    <div class="px-6 py-4 border-b border-[var(--border-subtle)]">
      <h2 class="text-sm font-semibold uppercase tracking-wider text-[var(--text-tertiary)]">
        {currentSection}
      </h2>
    </div>
  {/if}
